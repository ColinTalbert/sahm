...
#            }
    ...
    if(!is.null(out$error.mssg[[1]])){
      ...
      warning("Warning:I am warning you not to never,never,ever eat my gum again!!!")

       a<-try(glm(a,b),silent=TRUE)
       if(class(a)=="try-error"){
              if(!debug.mode) {sink();on.exit();unlink(paste("hello","_log.txt",sep=""))}
              out$ec<-out$ec+1
              out$error.mssg[[out$ec]] <- paste("a try catch error")
              cat(saveXML(brt.to.xml(out),indent=T),'\n')
              return()
              }
        #stop("YOU HAVE DONE A VERY, VERY BAD THING!!!!")
