Pairs.Explore<-function(num.plots=10,min.cor=.7,input.file,output.dir,response.col="ResponseBinary",cors.w.highest=FALSE){
  ...
  #before exploring predictor relationship
    ...
    match("site.weights",tolower(names(dat))),match(tolower(response.col),tolower(names(dat))))))
  ...
  #Remove columns with only one unique value
    dat<-dat[,as.vector(apply(dat,2,var)==0)!=1]
  ...
  #of those with which it is correlated
    ...
    HighToPlot<-dat[,names(dat)%in%names(High.cor[1:min(num.plots,length(High.cor))])]
      }else{
    ...
    cor.range<-c(min(abs(cor(HighToPlot))),1)
  ...
  ## with size proportional to the correlations.
      ...
      a<-colors()
          ...
          if(all.cor>=range.seq[18]){
            rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col =
            a[59])} else if(all.cor>=range.seq[14]){
            rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col =
            a[76])} else if(all.cor>=range.seq[10]){
            rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col =
            a[382])}
          r<-max(all.cor)
               cex.cor=3
...
#   }
  ...
  #Find a new unique file name (one in the desired directory that hasn't yet been used)
  outfile <- paste(output.dir,"Predictor_Correlation.jpg",sep="\\")
            while(file.access(outfile)==0) outfile<-paste(output.dir,"Predictor_Correlation.jpg",sep="\\")

        print(outfile)
            
 options(warn=-1)
  jpeg(outfile,width=1000,height=1000,pointsize=13)
    MyPairs(HighToPlot,cor.range=cor.range,my.labels=(as.vector(High.cor)[1:num.plots]),
    lower.panel=panel.smooth,diag.panel=panel.hist, upper.panel=panel.cor,pch=21,bg = c("red","steelblue")[as.factor(response)],col.smooth = "red")
  graphics.off()
 options(warn=0)
