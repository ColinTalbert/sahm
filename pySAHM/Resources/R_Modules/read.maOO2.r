 read.ma <- function(out){
      ...
      #########################################################################
       ...
       if(is.na(site.weights)) ma$weight=rep(1,times=dim(ma)[1])

        if(length(xy.cols)>0) out.list$xy$train<-ma[,xy.cols]
     ...
     #remove test training split column if present
          if(length(na.omit(split.indx))>0){
              ...
              out.list$dat$test<-ma[split.col=="test",]
              ma<-ma[split.col=="train",]
              out.list$weight$test<-out.list$weight$train[split.col=="test"]
              out.list$weight$train<-out.list$weight$train[split.col=="train"]
              out.list$xy$test<-out.list$xy$train[split.col=="test",]
              out.list$xy$train<-out.list$xy$train[split.col=="train",]
              out.list$n.pres[4] <- sum(out.list$dat$test[,r.col]!=0)
              out.list$n.abs[4] <- sum(out.list$dat$test[,r.col]==0)
            }
  ...
  #check that response column contains at least two unique values for counts
      ...
      if(tolower(out$input$model.family)=="poisson"){
         ...
         if(length(na.omit(split.indx))>0) out.list$dat$test <- cbind(out.list$dat$test[,r.col],out.list$dat$test[,-c(rm.list)])
                    paths<-paths[-c(rm.list)]
                    include<-include[-c(rm.list)]
                    
           out.list$resp.name <- names(ma)[1]<-"response"
           if(length(na.omit(split.indx))>0) names(out.list$dat$test)[1]<-"response"
         ma.names<-names(ma)
      ...
      # if producing geotiff output, check to make sure geotiffs are available for each column of the model array #
        if(out$input$make.binary.tif==T | out$input$make.p.tif==T){
         ...
         if(sum(file.access(paths),mode=0)!=0){
          ...
          out.list$dat$train <- ma

        out.list$dims <- dim(out.list$dat$train)
        out.list$ratio <- min(sum(out$input$model.fitting.subset)/out.list$dims[1],1)
        out.list$used.covs <- names(out.list$dat$train)[-1]

      if(!is.null(out$input$model.fitting.subset)){
            pres.sample <- sample(c(1:nrow(out.list$dat$train))[out.list$dat$train[,1]>=1],min(out.list$n.pres[2],out$input$model.fitting.subset[1]))
            abs.sample <- sample(c(1:nrow(out.list$dat$train))[out.list$dat$train[,1]==0],min(out.list$n.abs[2],out$input$model.fitting.subset[2]))
            out.list$dat$train.subset <- out.list$dat$train[c(pres.sample,abs.sample),]
            if(out$input$model.source.file!="rf.r") out.list$weight.subset<-out.list$weight$train[c(pres.sample,abs.sample)]
            out.list$n.pres[3] <- length(pres.sample)
            out.list$n.abs[3] <- length(abs.sample)
            } else {
            out.list$dat$train.subset <- NULL
            out.list$weight.subset<-NULL
            out.list$n.pres[3] <- NA
            out.list$n.abs[3] <- NA }

if(tolower(out$input$model.family)=="poisson"){
out.list$dat$train.subset<-out.list$dat$train
out.list$weight.subset<-out.list$weight$train
}
