...
#is into one group so the list structure can persist making the downstream coding easier
        ...
        if(out$input$PsdoAbs & floor(sum(dat$response==0)/sum(dat$response==1))>1){
                num.splits<-floor(sum(dat$response==0)/sum(dat$response==1))
                 ...
                 Split<-rep(seq(1,num.splits,by=1),length=sum(dat$response==0))
                 ...
                 Split<-rep(1,times= sum(dat$response==0))
                 num.splits<-1
       }
