...
fit.model <- function(ma.name,output.dir=NULL,response.col="^response.binary",make.p.tif=T,make.binary.tif=T,
responseCurveForm=NULL,debug.mode=F,model.family="binomial",script.name="glm.r",opt.methods=2,save.model=TRUE,UnitTest=FALSE,
MESS=FALSE,...){
     Call<-match.call()
     browser()
    ...
    out <- list(
      input= lapply(as.list(Call[2:length(Call)]),eval),
