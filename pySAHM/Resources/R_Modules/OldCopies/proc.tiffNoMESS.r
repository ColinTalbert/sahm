...
print(i)
  ...
  continuousRaster@data@min<-min.pred
  continuousRaster@data@max<-max.pred
  continuousRaster@file@nodatavalue<-NAval
  continuousRaster <- writeStop(continuousRaster)

  if(make.binary.tif) {
     binaryRaster@data@min<-0
     binaryRaster@data@max<-1
     binaryRaster@file@nodatavalue<-NAval
