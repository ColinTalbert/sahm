    ...
    capture.output(cat("\n\n============================================================",
          ...
          if(!is.null(out$dat$ma$ma.test))
                      ...
                      capture.output(cat( "\n",
                       ...
                       }else  x=data.frame(cbind(c("Correlation Coefficient","Percent Deviance Explained","Prediction Error","Sensitivity","Specificity"),
                            c(as.vector(cor.test(pred,response)$estimate),pct.dev.exp,prediction.error,SENS,SPEC)))
