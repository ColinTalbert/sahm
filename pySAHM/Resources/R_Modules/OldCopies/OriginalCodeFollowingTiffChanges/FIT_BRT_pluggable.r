...
fit.brt.fct <- function(ma.name,tif.dir=NULL,output.dir=NULL,response.col="^response.binary",test.resp.col="response",make.p.tif=T,make.binary.tif=T,
      simp.method="cross-validation",debug.mode=F,tc=NULL,n.folds=3,ma.test=NULL,alpha=1,script.name="brt.r"){    
    ...
    if(out$input$simp.method=="cross-validation"){
        ...
        t1 <- unclass(Sys.time())
        m0 <- try(gbm.step.fast(dat=out$dat$ma$ma.subset,gbm.x=out$mods$lr.mod$good.cols,gbm.y=1,family="bernoulli",
              n.trees = c(300,600,800,1000,1200,1500,1800),n.folds=out$input$n.folds,tree.complexity=out$mods$parms$tc.sub,
              learning.rate=out$mods$lr.mod$lr0,bag.fraction=out$input$bag.fraction,autostop=T,debug.mode=F,silent=T,
    ...
    # fit final model #
    t2 <- unclass(Sys.time())
    out$mods$final.mod <- try(gbm.step.fast(dat=out$dat$ma$ma,gbm.x=out$mods$simp.mod$good.cols,gbm.y = 1,family="bernoulli",
                  n.trees = c(300,600,700,800,900,1000,1200,1500,1800,2200,2600,3000,3500,4000,4500,5000),n.folds=out$input$n.folds,
                  tree.complexity=out$mods$parms$tc.full,learning.rate=out$mods$lr.mod$lr,bag.fraction=out$input$bag.fraction,
    ...
    if(out$input$make.p.tif==T | out$input$make.binary.tif==T){
        ...
        mssg <- try(proc.tiff(model=out$mods$final.mod,vnames=as.character(out$mods$final.mod$contributions$var),
            tif.dir=out$dat$tif.dir$dname,pred.fct=brt.predict,factor.levels=out$dat$ma$factor.levels,make.binary.tif=make.binary.tif,
            thresh=out$mods$auc.output$thresh,make.p.tif=make.p.tif,outfile.p=paste(out$dat$bname,"_prob_map.tif",sep=""),
            outfile.bin=paste(out$dat$bname,"_bin_map.tif",sep=""),tsize=50.0,NAval=-3000,logname=logname),silent=T)     #"brt.prob.map.tif"
...
est.lr <- function(out){
    ...
    while(max.trees<800 & i<=nrow(lr.out)){
       gbm.fit <- try(gbm.step.fast(dat=dat,
          gbm.x = gbm.x,
          gbm.y = gbm.y,
          family = "bernoulli",
          n.trees = n.trees,
          ...
          autostop=T,verbose=F,silent=T,plot.main=F),silent=T)
...
if(Debug==F){
    ...
    fit.brt.fct(ma.name=args[1],tif.dir=args[2],output.dir=args[3],script.name=script.name)
} 
if(Debug==T){
    if(batch.mode==T){
