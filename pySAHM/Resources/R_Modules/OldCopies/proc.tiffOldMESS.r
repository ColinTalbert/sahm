...
NAval<-as.double(-3.4e-38)
RasterInfo@file@nodatavalue<-NAval
  ...
  for (i in 1:tr$n) {
            ...
            if(is.null(dim(temp))){
              temp<- getValuesBlock(raster(fullnames[k]), row=tr$row[i], nrows=tr$size)
                  ...
                  if(MESS){
                        ...
                        pred.rng[,k]<-mapply(CalcMESS,tiff.entry=temp[,k],MoreArgs=list(pred.vect=pred.range))
                         }
            }
    ...
    temp[is.na(temp)]<-NA #this seemingly worthless line switches NaNs to NA so they aren't predicted
        if(!is.na(factor.levels)){
            ...
            pred.rng<-pred.rng[,-c(factor.cols)] #not currently considering factor columns because it's not obvious how to
        }
...
print(i)
    ...
    if(MESS) {MessRaster<-writeValues(MessRaster,apply(pred.rng,1,min), tr$row[i])
      browser()
      ModRaster
    }
