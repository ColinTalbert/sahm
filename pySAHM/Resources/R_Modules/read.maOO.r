 read.ma <- function(out){
     ...
     #remove test training split column if present
          ...
          if(length(na.omit(split.indx))>0){
              ...
              out.list$test<-ma[split.col=="test",]
              ...
              out.list$n.pres[4] <- sum(out.list$test[,r.col]!=0)
              out.list$n.abs[4] <- sum(out.list$test[,r.col]==0)
            }
  ...
  #check that response column contains at least two unique values for counts
      ...
      if(tolower(out$input$model.family)=="poisson"){
         ...
         if(length(na.omit(split.indx))>0) out.list$test <- cbind(out.list$test[,r.col],out.list$test[,-c(rm.list)])
                    paths<-paths[-c(rm.list)]
                    include<-include[-c(rm.list)]
                    
           out.list$resp.name <- names(ma)[1]<-"response"
           if(length(na.omit(split.indx))>0) names(out.list$test)[1]<-"response"
         ma.names<-names(ma)
      ...
      # if producing geotiff output, check to make sure geotiffs are available for each column of the model array #
        ...
        out.list$dims <- dim(out.list$train)
        out.list$ratio <- min(sum(out$input$model.fitting.subset)/out.list$dims[1],1)
        out.list$used.covs <- names(out.list$train)[-1]

      if(!is.null(out$input$model.fitting.subset)){
            pres.sample <- sample(c(1:nrow(out.list$train))[out.list$train[,1]>=1],min(out.list$n.pres[2],out$input$model.fitting.subset[1]))
            abs.sample <- sample(c(1:nrow(out.list$train))[out.list$train[,1]==0],min(out.list$n.abs[2],out$input$model.fitting.subset[2]))
            out.list$train.subset <- out.list$train[c(pres.sample,abs.sample),]
            ...
            out.list$train.subset <- NULL
            out.list$weight.subset<-NULL
            out.list$n.pres[3] <- NA
            out.list$n.abs[3] <- NA }

if(tolower(out$input$model.family)=="poisson"){
out.list$train.subset<-out.list$train
